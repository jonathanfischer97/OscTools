"""
    get_pairwise_diversity(population::AbstractMatrix{Float64})

Computes the maximum pairwise distance as a diversity metric for a population of individuals.
"""
function get_pairwise_diversity(population::AbstractMatrix{Float64})
    ...
    distances = pairwise(Euclidean(), normalized_population, dims=2)
    
    return maximum(distances)
end

function get_pairwise_diversity(population::Vector{Vector{Float64}})
    pop_matrix = stack(population)

    get_pairwise_diversity(pop_matrix)
end

function get_pairwise_diversity(df::DataFrame, exclude_cols::Vector{Symbol} = [:gen, :fit, :per, :amp, :relamp])
    dfmat = df_to_matrix(df, exclude_cols)
    return get_pairwise_diversity(dfmat)
end
